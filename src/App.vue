<template>
  <div class="app-container" @click="closeContextMenu">
    <!-- 启动加载遮罩：数据就绪前显示，避免窗口黑屏与空状态闪烁 -->
    <div v-if="isLoading" class="app-loading">
      <img class="app-loading-logo" :src="jueseBlackImg" alt="Carvis" />
      <div class="app-loading-spinner"></div>
      <div class="app-loading-text">正在启动 Carvis...</div>
    </div>
    ...
    <aside class="sidebar">
      <div class="sidebar-header">
        ...
        <span class="sidebar-version">v0.10.0</span>
      </div>
      ...
      <div class="sidebar-section">
        <div
          class="sidebar-section-title nav-item"
          :class="{ collapsed: !servicesExpanded }"
          @click="servicesExpanded = !servicesExpanded"
        >
          <span class="nav-item-label">服务</span>
          <span class="nav-item-count">({{ filteredServices.length }})</span>
        <svg class="nav-item-arrow" viewBox="0 0 16 16" width="11" height="11" fill="none" stroke="currentColor" stroke-width="1.4" stroke-linecap="round" stroke-linejoin="round">
          <path d="M6 4l4 4-4 4"/>
        </svg>
        </div>

        <div v-show="servicesExpanded" class="service-list">
          <!-- 按分组展示服务 -->
      ...
      <div v-if="killerPorts.length > 0" class="sidebar-section">
        <div
          class="sidebar-section-title nav-item"
          :class="{ collapsed: !portsExpanded }"
          @click="portsExpanded = !portsExpanded; openPortKiller()"
        >
          <span class="nav-item-label">端口清理</span>
          <span class="nav-item-count">({{ killerPorts.length }})</span>
        <svg class="nav-item-arrow" viewBox="0 0 16 16" width="11" height="11" fill="none" stroke="currentColor" stroke-width="1.4" stroke-linecap="round" stroke-linejoin="round">
          <path d="M6 4l4 4-4 4"/>
        </svg>
        </div>

        <div v-show="portsExpanded">
        <!-- 已固定端口：常驻左栏（服务项一致样式） -->
        <div v-if="pinnedPorts.length" class="pinned-list">
          <div
            v-for="port in pinnedPorts"
            :key="port"
            class="service-item pinned-port-item"
            @click="openPortKiller"
          >
            <span
              class="service-status-dot"
              :class="portStatuses[port] && portStatuses[port].occupied ? 'status-pinned' : 'status-pinned-free'"
            ></span>
            <div class="service-item-info">
              <span class="service-item-name">:{{ port }}</span>
            </div>
            <div class="service-item-actions" @click.stop>
              <button
                class="item-action-btn pinned-clean-btn"
                @click.stop="handlePinnedClean(port)"
                title="清理该端口"
              >
                <svg viewBox="0 0 16 16" width="12" height="12" fill="none" stroke="currentColor" stroke-width="1.6"><path d="M2 5h12M6 5V3h4v2M4 5l1 9h6l1-9"/></svg>
                <span class="action-btn-label">清除</span>
              </button>
            </div>
          </div>
        </div>
    ...
    <main class="main-content">
      ...
      <!-- 右侧内容：切换/进入时触发轻微自下向上移入动效（终端 Tab 切换服务时跳过） -->
      <Transition :name="skipViewAnimation ? 'view-no-anim' : 'view-enter'" mode="out-in" @after-enter="skipViewAnimation = false">
      <div class="right-view" :key="rightViewKey">
      <!-- 端口清理网格视图 -->
      <PortKiller
        v-if="showPortKiller"
        :ports="killerPorts"
        :pinned-ports="pinnedPorts"
        :statuses="portStatuses"
        :logs="portKillerLogs"
        @toggle-pin="handleTogglePin"
        @clean="handleCleanPort"
        @clean-all="handleCleanAll"
      />
    ...
    <!-- 轻量 Toast 提示：启动/停止结果等需要用户可见的反馈（打包后 console 被剥离） -->
...
<script setup>
import { ref, reactive, shallowReactive, computed, onMounted, onUnmounted, watch, nextTick, defineAsyncComponent } from 'vue'
import { tauriApi } from './services/tauriApi'
import TerminalPanel from './components/TerminalPanel.vue'
// R9: 按需懒加载——仅在用户打开「设置」或「端口清理」时动态拉取对应 chunk，
// 使主入口 chunk 体积下降，首屏/主流程渲染更快
const PortKiller = defineAsyncComponent(() => import('./components/PortKiller.vue'))
...
const services = ref([])
/** 启动加载态：数据就绪前显示遮罩，避免黑屏/空状态闪烁 */
const isLoading = ref(true)
const killerPorts = ref([])
const pinnedPorts = ref([])
const showPortKiller = ref(false)
...
// 点击终端 Tab 切换服务时跳过右侧视图的淡入上滑动效
const skipViewAnimation = ref(false)
...
// ========================
// 轻量 Toast 提示（打包后 console 被剥离，启动/停止结果必须可见）
// ========================
...
function serviceLabel(id) {
  return services.value.find((s) => s.id === id)?.label || id
}
...
watch(terminalTab, (newTab) => {
  ...
  if (services.value.some(s => s.id === newTab)) {
    // 终端 Tab 切换服务时不触发淡入上滑动效；
    // 标志由 <Transition @after-enter> 在整个过渡结束后才复位，
    // 避免 enter 阶段时标志已被提前清除而重新触发动画。
    skipViewAnimation.value = true
...
const portKillerLogs = computed(() => logs['port-killer'] || [])

/** 右侧视图 key：每次进入/切换右侧内容时变化，触发轻量自下向上移入动效 */
const rightViewKey = computed(() => {
  if (showPortKiller.value) return 'port-killer'
  if (!serviceSelection.value) return 'empty'
  return 'service-' + serviceSelection.value
})
...
function initFromConfig(config) {
  ...
  pinnedPorts.value = config.pinnedPorts || []
...
async function confirmRename() {
  try {
    ...
    if (renameModal.type === 'group') {
      ...
      const result = await tauriApi.saveConfig(JSON.parse(JSON.stringify(currentConfig.value)))
      if (result.success) {
...
function handleOpenFolderFromMenu() {
  if (contextMenu.service && contextMenu.service.cwd) {
    tauriApi.openFolder(contextMenu.service.cwd)
  }
  closeContextMenu()
}
...
onMounted(async () => {
  // 关键：先注册事件监听，确保后续「启动/停止」发出的状态与日志事件一定能到达前端。
  // 必须放在 showWindow 之前——否则若 showWindow 的 invoke 在生产环境偶发异常，
  // finally 之后的监听注册代码会被整体跳过，导致启动后 UI 永远收不到状态更新（表现为“点击无反应”）。

  // 监听服务日志
  unsubscribeLog = tauriApi.onLog((data) => {
  ...
  // 监听状态变更
  unsubscribeStatus = tauriApi.onStatusChange((data) => {
    statuses[data.service] = data.status
  })

  // 监听应用关闭准备事件
  unsubscribeShutdown = tauriApi.onAppShutdownReady(() => {
    console.log('App shutdown ready')
  })

  // 检测平台
  if (tauriApi.platform) {
    isMacPlatform.value = tauriApi.platform.isMac
  ...
  try {
    ...
    const [settingsResult, configResult] = await Promise.all([
      tauriApi.getSettings().catch(() => ({ success: false })),
      tauriApi.getFullConfig().catch(() => ({ success: false })),
    ])
    ...
    tauriApi.getAllStatus().then(statusResult => {
    ...
    // 数据就绪（或兜底失败）后结束加载态，并显示窗口
    // 此时 Vue 已渲染真实 UI，遮罩移除后再 show 窗口，避免黑屏/空状态闪烁
    isLoading.value = false
    // showWindow 失败不应中断流程：窗口已由 Rust 的 4s 兜底逻辑强制显示，
    // 且上面的事件监听已注册，避免“因 showWindow 异常而跳过监听注册”的回归。
    tauriApi.showWindow().catch(() => {})
  }
})
...
async function handleStart(serviceId) {
  ...
  const result = await tauriApi.start(serviceId)
  if (result.success) {
    pushToast(`已发送启动请求：${label}`, 'success')
  } else {
    // 生产环境 console 被剥离，必须用可见提示暴露失败原因（如命令未找到 / PATH 问题）
    pushToast(`启动失败（${label}）：${result.error || '未知错误'}`, 'error', 6000)
  }
}

async function handleStop(serviceId) {
  const label = serviceLabel(serviceId)
  const result = await tauriApi.stop(serviceId)
  if (result.success) {
    pushToast(`已发送停止请求：${label}`, 'info')
  } else {
    pushToast(`停止失败（${label}）：${result.error || '未知错误'}`, 'error', 6000)
  }
}
...
async function handleRestart(serviceId) {
  ...
  if (st === ServiceStatus.RUNNING || st === ServiceStatus.STARTING) {
    await tauriApi.stop(serviceId)
    await new Promise(r => setTimeout(r, 1500))
  }
  await tauriApi.start(serviceId)
}
...
/** 打开右侧端口清理网格视图 */
function openPortKiller() {
  showPortKiller.value = true
}

/** 刷新单个端口的占用状态 */
async function refreshPortStatus(port) {
  try {
    const res = await tauriApi.getPortStatus(port)
    if (res && res.success && res.data) {
      portStatuses[port] = res.data
    } else {
      portStatuses[port] = { occupied: false, pids: [], processCount: 0 }
    }
  } catch (e) {
    portStatuses[port] = { occupied: false, pids: [], processCount: 0 }
  }
...
/** 刷新所有端口的占用状态 */
async function refreshAllPortStatuses() {
  await Promise.all((killerPorts.value || []).map(p => refreshPortStatus(p)))
}

/** 切换端口固定状态（固定后常驻左栏） */
async function handleTogglePin(port) {
  const isPinned = pinnedPorts.value.includes(port)
  const res = isPinned
    ? await tauriApi.unpinPort(port)
    : await tauriApi.pinPort(port)
  if (res && res.success && res.data) {
    pinnedPorts.value = res.data.pinned || []
  }
}

/** 取消固定（左栏迷你卡片调用） */
async function handleUnpin(port) {
  const res = await tauriApi.unpinPort(port)
  if (res && res.success && res.data) {
    pinnedPorts.value = res.data.pinned || []
  }
}
...
/** 左栏迷你卡片：清理固定端口 */
async function handlePinnedClean(port) {
  await tauriApi.killPort(port)
  await refreshPortStatus(port)
}

/** 网格内：清理单个端口 */
async function handleCleanPort(port) {
  await tauriApi.killPort(port)
  await refreshPortStatus(port)
}

/** 网格内：全部清理（含固定端口，由后端保证） */
async function handleCleanAll() {
  const res = await tauriApi.cleanAllPorts()
  if (res && res.success) {
    await refreshAllPortStatuses()
  }
}
...
async function handleSaveConfig(payload) {
  ...
  try {
    ...
    // 深拷贝为纯对象，避免 Vue 响应式代理触发 IPC 结构化克隆报错
    // ("An object could not be cloned")
    const plainConfig = JSON.parse(JSON.stringify(nextConfig))
    const result = await tauriApi.saveConfig(plainConfig)
    ...
    if (result && result.success) {
      ...
      if (nextSettings && typeof nextSettings.autoStart !== 'undefined') {
        ...
        tauriApi.setAutoStart(nextSettings.autoStart).catch(e =>
...
async function openFolder(folderPath) {
  await tauriApi.openFolder(folderPath)
}
</script>
