...
onMounted(async () => {
  // 获取版本号
  try {
    const version = await window.serviceAPI.getAppVersion()
    appVersion.value = version || '1.0.2'
  } catch {
...
/**
 * 持久化启动设置到后端（仅在用户点击"保存设置"时调用）
 * 同步更新开机自启动的 OS 启动项
 */
async function persistStartupSettings() {
  const settingsResult = await window.serviceAPI.getSettings()
  const settings = settingsResult.success ? settingsResult.data : {}

  settings.autoStart = startupSettings.autoStart
  settings.minimizeToTray = startupSettings.minimizeToTray

  await window.serviceAPI.saveSettings(settings)
  await window.serviceAPI.setAutoStart(startupSettings.autoStart)
}
